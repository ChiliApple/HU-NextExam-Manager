...
- Läuft unbeaufsichtigt (für Dauerbetrieb)
- Client Secret DPAPI-verschlüsselt in `%APPDATA%\HU-NextExam\mdm_{tenantId}.cred`
- Benötigt: Entra ID App Registration mit Application Permissions + Client Secret

### Modus B: Admin-Anmeldung (interaktiv)

- Auth Code Flow mit PKCE via Browser-Login
- Token gilt nur für die laufende Session
- Benötigt: Entra ID App Registration mit Delegated Permissions
- Empfohlen für Ersteinrichtung und Tests

Welcher Modus verwendet wird, wählst du im MDM-Tab per Radio-Button.
...
1. Tool starten, **MDM-Tab** öffnen
2. **"+ Tenant"** — Namen eingeben
3. **Radio-Button auf "Admin-Anmeldung (diese Session)"** umschalten
4. **"Verbinden"** klicken — Browser öffnet sich, mit **Global Admin** anmelden
5. Im Browser-Dialog: Checkbox **"Consent on behalf of your organization"** aktivieren → **Accept**
6. Zurück im Tool: Status zeigt "Verbunden (Admin-Token)"
7. Jetzt **"App einrichten"** klicken
8. Bestätigen — das Tool erstellt automatisch die App Registration
...
10. Credentials werden automatisch DPAPI-verschlüsselt gespeichert
11. Radio-Button zurück auf **"App-Credentials (automatisch)"** → **"Verbinden"** — fertig
...
> müssen über den Admin Consent Dialog im Browser erteilt werden. Ohne diesen
> Schritt schlägt die App-Erstellung mit "consent_required" fehl.

### Option 2: Manuell via Azure Portal

Falls der In-App Setup nicht möglich ist (z.B. Conditional Access blockiert localhost):
...
12. MDM-Tab — Tenant auswählen — **"Verbinden"**
13. Beim ersten Verbinden fragt das Tool nach Client ID + Tenant ID + Secret
14. Credentials werden DPAPI-verschlüsselt gespeichert
...
1. MDM-Tab — Tenant auswählen — **"Verbinden"**
...
- **"Status prüfen"** — vergleicht Metadaten zwischen GitHub-Release und Intune-App
...
Jeder M365-Tenant braucht eine eigene App Registration. Im Tool können beliebig
